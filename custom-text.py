...
def pad(data: bytes, block_size: int = 16) -> bytes:
    # Calculate the number of padding bytes needed to make the data length a multiple of the block size
    pad_len = block_size - len(data) % block_size
    # Append the padding bytes to the data, where each byte's value is equal to the number of padding bytes
...
def unpad(data: bytes) -> bytes:
    ...
    pad_len = data[-1]
    # Remove the padding bytes from the end of the data
    return data[:-pad_len]
...
if __name__ == "__main__":
    ...
    plaintext = "mai-moahmmed".encode("utf-8")  # The original plaintext
    padded_plaintext = pad(
        plaintext
    )  # Pad the plaintext to ensure it is a multiple of the block size
    key = bytearray.fromhex("000102030405060708090a0b0c0d0e0f")
    ciphertext = aes_encryption(padded_plaintext, key)  # Encrypt the padded plaintext
    recovered_padded_plaintext = aes_decryption(
        ciphertext, key
    )  # Decrypt to get the padded plaintext back
    recovered_plaintext = unpad(
        recovered_padded_plaintext
    )  # Remove the padding to recover the original plaintext

    print(
        "Original Plaintext (hex):", plaintext.hex()
    )  # Display the original plaintext in hex
    print(
        "Padded Plaintext (hex):", padded_plaintext.hex()
    )  # Display the padded plaintext in hex
    print("Key (hex):", key.hex())  # Display the encryption key in hex
    print("Ciphertext (hex):", ciphertext.hex())  # Display the ciphertext in hex
    print(
        "Recovered Plaintext (utf-8):", recovered_plaintext.decode("utf-8")
    )  # Display the recovered plaintext in utf-8

    # AES-192
    print("\nTesting AES-192...")
    plaintext = "mai-moahmmed".encode("utf-8")  # The original plaintext
    padded_plaintext = pad(plaintext)  # Pad the plaintext
    ...
    print("Original Plaintext (hex):", plaintext.hex())
    print("Padded Plaintext (hex):", padded_plaintext.hex())
    print("Key (hex):", key.hex())
    print("Ciphertext (hex):", ciphertext.hex())
    print("Recovered Plaintext (utf-8):", recovered_plaintext.decode("utf-8"))
